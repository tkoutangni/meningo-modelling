...
yearSpecFit<-function(district_id, district_year_data, year_now,
                        ...
                        nloptrAlgorithm, # must be of string character type
                        show_plot = FALSE,
                        n_iter = 1000) {
        ...
        for (i in c(hc_vector)) {
                ...
                Objective_least_square <-
                        function(guess_parms, parmset = names(guess_parms)) {
                                ...
                                print(guess_parms)
                                #print(current_parms_combination)
                ...
                guess_parms = initial_guess_parms # intial_guess_parms vector is in the parameters R file.
                ...
                fit_algo <- function(useFME = FALSE,
                        ...
                        if (useFME) {
                                Fit <- modFit(
                                        p = guess_parms, f = Objective_least_square, method = "L-BFGS-B",
                                        control = list(
                                ...
                                Fit <-
                                        nloptr(
                                                eval_f = Objective_least_square, # the objective function
                                                x0 = guess_parms,
                                                parmset = names(guess_parms),
                                                eval_g_ineq = ineqConstrain, # the inequality constrain function
                                                opts = list(
                                                        "algorithm" = nloptrAlgorithm, "print_level" = 0,
                ...
                fitted_parms[names(Fit$par)] <-
                        Fit$par # replacing the unknown parameters values with estimated values
                
                print(cbind(fitted_parms, vparameters))
                ...
                if (show_plot == TRUE) {
                        ...
                        if (printDetail) {
                                ...
                                text(
                                        x = 45,y = max_value_data , bquote(bold(
                                                "Estim Param (day-1)"
                                        )),cex = .7,adj =
                                                1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[1] , bquote("Transmision" == .(
                                                Fit$par["beta0"]
                                        )),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[2] , bquote(
                                                "Carriage lost" == .(Fit$par["alpha"])
                                        ),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[3] , bquote("Imunity lost" == .(
                                                Fit$par["phi"]
                                        )),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[4],  bquote("Peak timing" == .(
                                                Fit$par["teta"]
                                        )),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[5],  bquote(
                                                "Forcing ampl_a" == .(Fit$par["epsilon_a"])
                                        ),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[6],  bquote(
                                                "Forcing ampl_b" == .(Fit$par["epsilon_b"])
                                        ),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[7] , bquote(S[0] == .(
                                                Fit$par["Susc0"]
                                        )),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[8], bquote(C[0] == .(
                                                Fit$par["CarrierProp"]
                                        )),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 45,y = max_value_data - minus_x[9] , bquote(
                                                "Invasion rate" == .(Fit$par["a0"])
                                        ),cex =
                                                .7,adj = 1
                                )
                                ...
                                text(
                                        x = 25,y = max_value_data , bquote(bold(
                                                "Incid (x10^5)"
                                        )),cex = .7,adj =
                                                1
                                )
                                text(
                                        x = 25,y = max_value_data - minus_x[1] , bquote(Min == .(
                                                incid_range[1]
                                        )),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 25,y = max_value_data - minus_x[2], bquote(Max == .(
                                                incid_range[2]
                                        )),cex =
                                                .7,adj = 1
                                )
                                ...
                                carr_range = round(range(fitted_model_all_var["Carrier"]) *
                                                           100,3)
                                text(
                                        x = 25,y = max_value_data - minus_x[3] , bquote(bold(
                                                "Carriage (%)"
                                        )),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 25,y = max_value_data - minus_x[4] , bquote(Min == .(
                                                carr_range[1]
                                        )),cex =
                                                .7,adj = 1
                                )
                                text(
                                        x = 25,y = max_value_data - minus_x[5], bquote(Max == .(
                                                carr_range[2]
                                        )),cex =
                                                .7,adj = 1
                                )
                        ...
                        range_a = range(at(Fit$par["a0"] * year,Fit$par["epsilon_a"],Fit$par["teta"]))
                        range_b = range(betat(Fit$par["beta0"] * year,Fit$par["epsilon_b"],Fit$par["teta"])) # multiply beta0*year
        ...
        return(fit_out)
}# yearSpecFit least-square fitting function for model without age structure ends
...
mleYearSpecFit <-
        function(district_id, district_year_data, year_now,
                ...
                for (i in c(hc_vector)) {
                        ...
                        parnames(Objective_max_likelihood) <-
                                names(guess_parms)
                        ...
                        fit_algo <-
                                function(useFME = FALSE, useMLE = FALSE,
                                         guess_parms = guess_parms,
                                         lbound = lower_bound,
                                         ubound = upper_bound) {
                                        useFME = useFME
                                        ...
                                        if (useFME) {
                                                Fit <- modFit(
                                                        p = guess_parms, f = Objective_max_likelihood, method = algorithm, #"L-BFGS-B"
                                                        control = list(
                                                                maxit = n_iter,trace = 6,factr = 1e-14
                                                        ), #
                                                        lower = lbound,
