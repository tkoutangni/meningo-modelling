    ...
    for (i in c(hc_vector)) {
        ...
        Objective_least_square <-
            function(guess_parms, parmset = names(guess_parms)) {
                ...
                mc = modCost(obs = obs.data, model = out, weight = "std") #, weight="mean", weight = "std"
                #print(mc$model)
                return(mc$model)
        ...
        if (show_plot == TRUE) {
            ...
            title(
                main = paste(hs_name),
                xlab = paste("Calendar weeks",year_now),ylab = "Incidence cases" # Incid x100,000
            )
            ...
            if (printDetail) {
                ...
                text(
                    x = 45,y = max_value_data - minus_x[1] , bquote("Transmision" == .(Fit$par["beta0"])),cex =
                        .7,adj = 1
                )
                text(
                    x = 45,y = max_value_data - minus_x[2] , bquote("Carriage lost" == .(Fit$par["alpha"])),cex =
                        .7,adj = 1
                )
                text(
                    x = 45,y = max_value_data - minus_x[3] , bquote("Imunity lost" == .(Fit$par["phi"])),cex =
                        .7,adj = 1
                )
                text(
                    x = 45,y = max_value_data - minus_x[4],  bquote("Peak timing" == .(Fit$par["teta"])),cex =
                        .7,adj = 1
                )
                text(
                    x = 45,y = max_value_data - minus_x[5],  bquote("Forcing ampl_a" == .(Fit$par["epsilon_a"])),cex =
                        .7,adj = 1
                )
                text(
                    x = 45,y = max_value_data - minus_x[6],  bquote("Forcing ampl_b" == .(Fit$par["epsilon_b"])),cex =
                        .7,adj = 1
                )
                text(
                    x = 45,y = max_value_data - minus_x[7] , bquote(S[0] == .(Fit$par["Susc0"])),cex =
                        .7,adj = 1
                )
                text(
                    x = 45,y = max_value_data - minus_x[8], bquote(C[0] == .(Fit$par["CarrierProp"])),cex =
                        .7,adj = 1
                )
                text(
                    x = 45,y = max_value_data - minus_x[9] , bquote("Invasion rate" == .(Fit$par["a0"])),cex =
                        .7,adj = 1
                )
