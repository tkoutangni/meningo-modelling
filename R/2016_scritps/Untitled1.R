...
yearSpecFit <- function(district_id, district_year_data, year_now,
    ...
    for (i in c(hc_vector)) {
        ...
        fit_algo <- function(useFME = FALSE,
            ...
            if (useFME) {
                Fit <- modFit(
                    p = guess_parms, f = Objective, method = "L-BFGS-B",
                    control = list(
                ...
                Fit <-
                    nloptr(
                        eval_f = Objective, # the objective function
                        x0 = guess_parms,
        ...
        if (show_plot == TRUE) {
            ...
            plot(
                obs.data[,2] * 1e+05,las = 1,pch = 20, col = "black",xlab = "",type = "p",ylim =
                    c(0,(
                        1.1 * max(obs.data[,2] * 1e+05,na.rm = TRUE)
                    )),ylab = ""
            ) #ylim=c(0,(1.1*max(obs.data[-c(na.data.index),2]*1e+05)))
            lines(
                fitted_model * 1e+05,col = "red",type = "l",pch = 20,lwd = 2
            )
