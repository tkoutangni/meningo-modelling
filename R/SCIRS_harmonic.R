...
SCIRS_harmonic = function(t, x, vparameters){
    Susc    = as.numeric(x[1])  # the value of S at time t
    Carrier = as.numeric(x[2])  # the value of C at time t
    Ill     = as.numeric(x[3])  # the value of I at time t
    Recov   = as.numeric(x[4])  # the value of R at time t
...
sim.SCIRS_harmonic <- function(inits, vparameters, nd){
    # @ parameters: 
    ...
    if(sum(names(vparameters)%in%"Susc0")>0){
        ...
        #inits["Carrier"]=vparameters["CarrierProp"]
    ...
    SCIRS_harmonic.out = as.data.frame(lsoda(y=inits, times=t_range, 
                                             func=SCIRS_harmonic, parms=vparameters))  
    # Gross approximation of weekly incidences from daily estimates of state  variables.
    # daily prediction of newI are somewhat similar within a given week.
    SCIRS_harmonic.out[,6] = SCIRS_harmonic.out[,6]*week 
    return(SCIRS_harmonic.out)
