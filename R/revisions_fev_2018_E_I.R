#revisions february 2018 for Epidemiology and infections
source("R/run_first.R")
source("R/models_parameters.R")
# counts of health center per district in a given year
sapply(non_epi_health_center_year_data, dim)[2,]

sum(sapply(non_epi_health_center_year_data, dim)[2,]) #=> 196

# visualize all hc data to see which was not used

pdf(file = "R/all_non_epi_hc_year_data.pdf")
ggplot_time_series_all_years(hyperendemic_data_to_plot)
 lapply(non_epi_health_center_year_data, plot_time_serie)
dev.off()

#  health center names per distric year
sapply(non_epi_health_center_year_data, colnames)
# unique health center names
unique(unlist(sapply(non_epi_health_center_year_data, colnames)))
length(unique(unlist(sapply(non_epi_health_center_year_data, colnames))))
