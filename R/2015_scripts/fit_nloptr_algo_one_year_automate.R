...
##=======================================================================================================================
#IMPOERTANT : BOTH the value of a0Forcing and beta0Forcing must be set carfully depending on whether we want to force
# the invasion rate alone or the transmission alone or both at a time.
##=======================================================================================================================
...
yearSpecFit<-function(district_id, district_year_data, year_now, hc_vector){
    hc_vector = hc_vector
    # Set a matrix to record the parameters estimates and other useful variables 
    fit_out<-matrix(NA,ncol=14,nrow=length(hc_vector),byrow=TRUE,
                    dimnames = list(c(hc_vector),c("district","hs","year","beta0","alpha","phi","Susc0","CarrierProp","teta","epsilon_a","epsilon_b","a0","f_val","Rsquared")))
    
    # remoove or add a parameter to estimate from/to the fit_out matrix depending on which of the transmission rate and invasion rate is to be forced
    ...
    }else if(a0Forcing==FALSE & beta0Forcing==FALSE){fit_out=fit_out}
    ...
    for (i in c(hc_vector)){ # for loop start here
        ...
        #Optimisation using modFit from FME package or nloptr from the nloptr package
        
        fit_algo <- function(useFME = FALSE, guess_parms, lbound, ubound){
            useFME = useFME
            if(useFME){
        ...
        na.data.index = which(is.na(obs.data$incid)) # to be used to set the max value of y-axis of the plotting area
        ...
        hs_name = names(ready_data)[hcIndex] # selecting health center name
        title(main=paste(hs_name),
              xlab=paste("Calendar weeks",year_now),ylab="Incid x100,000")
        } # end of the ploting of model versus data
        
        #computing the R-squared coef
        meanObsData = mean(obs.data[,2],na.rm=TRUE)
        totalSumOfSquares = sum((fitted_model - meanObsData)^2)
        sumOfSquaredError = Fit$objective
        Rsquared = 1 - (sumOfSquaredError/totalSumOfSquares)
        ...
        fit_data = append(c(district_id,hcIndex,as.numeric(year_now),Fit$objective,Rsquared),Fit$solution,after=3)
