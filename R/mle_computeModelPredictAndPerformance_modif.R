...
mle_computeModelPredictionAndPerformStat <-
        function(paramEstimatesMatrice) {
                ...
                for (i in seq_along(paramEstimatesMatrice[, "district"])) {
                        ...
                        # this function code as more health center year data becomes availaible.
                        select_data <- function(year_now, district_now) {
                                if (year_now == 2006 & district_now == 1) {
                                        ...
                                        data = Kvigue_2010
                                        N = populations_in_this_district(Kvigue_2010, "2010")
                                }
                        ...
                        if (doPlot) {
                                ...
                                add_hc_name = add_hc_name
                                title(
                                        main = ifelse(add_hc_name, paste(hc_name), paste("")), #paste(hc_name),
                                ...
                                #ifelse(add_carriage, addCarriagePlot(), NA)  # call the function
                                ...
                                addFoldIncrease = function(range_a = NA,
                                        ...
                                        #a_fold = round((max(range_a) - min(range_a))/ mean(range_a), 1)
                                        #beta_fold = round(max(range_b) / mean(range_b), 1)
                                        beta_fold = round((max(range_b) - min(range_b))/ mean(range_b), 1)
                        ...
                        #a_fold[i] <- round((max(range_a) - min(range_a))/ mean(range_a), 1)
                        #beta_fold[i] <- round(max(range_b) / mean(range_b), 1)
                        beta_fold[i] = round((max(range_b) - min(range_b))/ mean(range_b), 1)
                } # end for loop
